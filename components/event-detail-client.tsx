...
import { MoreVertical, Copy, Download, Users, Trash2, Edit } from "lucide-react"
...
export function EventDetailClient({ event, slots, signups, waitlist, showCopyButton }: EventDetailClientProps) {
    ...
    <>
      <div className="bg-muted/50 border border-border rounded-lg p-4">
        <div className="flex items-center justify-between gap-4">
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-muted-foreground truncate">{signupUrl}</p>
          </div>
          <Button variant="secondary" size="sm" onClick={handleCopyLink}>
            <Copy className="w-4 h-4 mr-2" />
            Copy Link
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{slots.reduce((acc, s) => acc + s.available, 0)}</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
      ...
      <div className="flex justify-end">
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            <DropdownMenuItem onClick={handleCloseEvent} disabled={isClosing || event.status === "closed"}>
              {event.status === "closed" ? "Event Closed" : isClosing ? "Closing..." : "Close Event"}
            </DropdownMenuItem>
            <DropdownMenuItem
