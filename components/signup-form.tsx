      ...
      if (response.ok) {
        if (data.isWaitlist) {
          ...
          toast({
            title: "Success!",
            description:
              "You've been signed up successfully. Check your email for confirmation.",
          });
        }
        try {
          onSuccess?.({
          ...
          <div className="flex gap-2">
            ...
            {onBack && (
              <Button
                ...
                onClick={onBack}
                disabled={isSubmitting}
                className="border-slate-300 dark:border-white/20 hover:bg-slate-100 dark:hover:bg-white/10 font-semibold"
              >
                Back
              </Button>
            )}
