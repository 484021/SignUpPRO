...
export default async function SignupPage({
  ...
  const occurrenceDate = resolvedSearchParams.occurrence_date as string | undefined
  ...
  const { data: organizer } = await supabase.from("users").select("plan").eq("id", event.user_id).single()

  let slotsQuery = supabase.from("slots").select("*").eq("event_id", event.id).order("created_at", { ascending: true })
      ...
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        <Card className="w-full max-w-md text-center">
          <CardContent className="pt-6">
            ...
            <p className="text-muted-foreground">This event doesn't have any slots configured.</p>
          </CardContent>
        </Card>
      </div>
    )
