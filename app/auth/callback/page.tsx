    ...
    const handleCallback = async () => {
      try {
        ...
        if (code) {
          ...
          if (data?.session) {
            console.log("[v0] Session established for user:", data.session.user.email)
        ...
        if (session) {
          console.log("[v0] Existing session found for user:", session.user.email)
          console.log("[v0] Redirecting to dashboard...")
          router.replace("/dashboard")
        } else {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-background">
      <div className="text-center">
        ...
        <p className="text-xs text-muted-foreground mt-2">You will be redirected to the dashboard</p>
      </div>
    </div>
  )
}
